...
def get_header_files_in_folder(owner, repo, folder_path):
    ...
    if response.status_code == 200:
        ...
        for item in content:
            if 'type' in item and item['type'] == 'file' and item['name'].endswith('.h'):
...
files_in_folder =  get_header_files_in_folder(CAN_lib_owner, CAN_lib_repo, CAN_lib_folder)
...
def get_bms_coulomb_count_signals():
    signals = []
    ch_disch_conv = conversion.LinearConversion(scale=(1/10000), offset=0, is_float=False)
    signals.append(can.Signal(name="total charge", start=0, length=32, conversion=ch_disch_conv))
    signals.append(can.Signal(name="total discharge", start=32, length=32, unit="Coulombs", conversion=ch_disch_conv))
    return signals

def get_bms_detailed_temp_signals():
    signals = []
    temp_scale_conv = conversion.LinearConversion(scale=(1/100), offset=0, is_float=False)
    signals.append(can.Signal(name="ic id", start=0, length=4))
    signals.append(can.Signal(name="group id", start=4, length=4))
    id=0
    while id < 3:
        signals.append(can.Signal(name="thermistor id "+id, start=8+(16*id), length=16, conversion=temp_scale_conv))
        id+=1
    return signals
